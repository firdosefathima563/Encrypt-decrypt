...
// ── BACKEND HISTORY SAVING ────────────────────────────────────────────────────
// Best-effort save to the live backend (Render + Supabase). Never blocks the
// UI and never throws — if the backend is asleep, slow, or unreachable, the
// site keeps working exactly as before; the save just silently fails.
...
function saveToHistory(type, payload) {
  if (!API_URL) return; // backend not configured — skip silently
  fetch(`${API_URL}/api/history/save`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ type, payload, sessionId: getSessionId() }),
  }).catch(() => {
    /* offline or backend asleep — fine, this is best-effort only */
  });
}
...
function CaesarCipher() {
  ...
  // Debounced save — waits until the user pauses (1s) before logging to
  // history, so typing doesn't spam the backend with a request per keystroke.
  useEffect(() => {
    if (!input) return;
    const timer = setTimeout(() => {
      saveToHistory("caesar", { input, shift, mode, output });
    }, 1000);
    return () => clearTimeout(timer);
  }, [input, shift, mode, output]);
...
function PasswordStrength() {
  ...
  // Debounced save — summary metrics ONLY (score, strength, entropy, length).
  // The raw password is never sent or stored, matching the UI's own promise
  // that passwords are "analyzed locally — never sent anywhere."
  useEffect(() => {
    if (!pwd) return;
    const timer = setTimeout(() => {
      saveToHistory("password", { length: pwd.length, score, strength, entropy, crackTime });
    }, 1200);
    return () => clearTimeout(timer);
  }, [pwd, score, strength, entropy, crackTime]);
  const checkItems = [
...
function HistoryViewer() {
  ...
  const load = () => {
    ...
    fetch(`${API_URL}/api/history${qs}`, {
      headers: { "X-Session-Id": getSessionId() },
    })
      .then(r => r.json())
      .then(data => { setEntries(data.entries || []); setStatus("ok"); })
      .catch(() => setStatus("error"));
  };

  useEffect(() => { load(); /* eslint-disable-next-line */ }, [filter]);

  const typeIcon = { rsa: "🔑", caesar: "🔠", password: "🛡️" };
  const typeColor = { rsa: C.indigo, caesar: C.amber, password: C.purple };

  return (
    <section id="history" style={{ padding: "7rem 2rem", background: C.bgAlt }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <SectionHeading tag="Backend · Saved History" title="Recent Activity" subtitle="Every RSA run, Caesar cipher, and password check above is logged to a live database (Supabase) through the SecureCrypt backend API." />
        <div style={{ display: "flex", justifyContent: "center", gap: "8px", marginBottom: "1.5rem", flexWrap: "wrap" }}>
          {["all", "rsa", "caesar", "password"].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              style={{ background: filter === f ? C.indigoLight : C.surface, border: `1.5px solid ${filter === f ? C.indigo : C.border}`, color: filter === f ? C.indigo : C.muted, padding: "5px 16px", borderRadius: 8, cursor: "pointer", fontSize: "0.78rem", fontWeight: filter === f ? 700 : 400, textTransform: "capitalize" }}>
              {f}
            </button>
          ))}
          <button onClick={load} style={{ background: C.surface, border: `1.5px solid ${C.border}`, color: C.muted, padding: "5px 16px", borderRadius: 8, cursor: "pointer", fontSize: "0.78rem" }}>
            ↻ Refresh
          </button>
        </div>

        {status === "error" && (
          <div style={{ ...card, padding: "2rem", textAlign: "center", color: C.muted, fontSize: "0.85rem" }}>
            Couldn't reach the backend right now. If it's been idle, Render's free tier can take up to a minute to wake up — try refreshing in a moment.
          </div>
        )}
        {status === "loading" && (
          <div style={{ ...card, padding: "2rem", textAlign: "center", color: C.muted, fontSize: "0.85rem" }}>Loading history…</div>
        )}
        {status === "ok" && entries.length === 0 && (
          <div style={{ ...card, padding: "2rem", textAlign: "center", color: C.muted, fontSize: "0.85rem" }}>
            Nothing saved yet — try the RSA simulator, Caesar cipher, or password checker above.
          </div>
        )}
        {status === "ok" && entries.length > 0 && (
          <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
            {entries.map(e => (
              <div key={e.id} style={{ ...card, padding: "1rem 1.25rem", display: "flex", gap: "12px", alignItems: "flex-start" }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, background: `${typeColor[e.type]}15`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.95rem", flexShrink: 0 }}>{typeIcon[e.type] || "📄"}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 3 }}>
                    <Tag color={typeColor[e.type]} bg={`${typeColor[e.type]}15`}>{e.type?.toUpperCase()}</Tag>
                    <span style={{ fontSize: "0.68rem", color: C.muted }}>{new Date(e.created_at).toLocaleString()}</span>
                  </div>
                  <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.75rem", color: C.textMid, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {e.type === "rsa" && `"${e.payload.message}" → ${e.payload.match ? "✅ matched" : "❌ mismatch"} (${e.payload.bits}-bit)`}
                    {e.type === "caesar" && `${e.payload.mode} shift ${e.payload.shift}: "${e.payload.input?.slice(0, 40)}"`}
                    {e.type === "password" && `${e.payload.strength} · ${e.payload.entropy} bits · ${e.payload.length} chars`}
                  </div>
                </div>
              </div>
            ))}
...
const ALL_SECTIONS = ["hero", "simulator", "primes", "demo", "cipher", "password", "history"];
